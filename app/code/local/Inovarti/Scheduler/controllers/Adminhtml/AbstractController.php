 ...
 */
abstract class Inovarti_Scheduler_Adminhtml_AbstractController extends Mage_Adminhtml_Controller_Action {
	...
	protected function checkHeartbeat() {
		if (!Mage::helper('inovarti_scheduler')->isDisabled('inovartischeduler_heartbeat')) {
			$lastHeartbeat = Mage::helper('inovarti_scheduler')->getLastHeartbeat();
			if ($lastHeartbeat === false) {
				...
				$timespan = Mage::helper('inovarti_scheduler')->dateDiff($lastHeartbeat);
				if ($timespan <= 5 * 60) {
