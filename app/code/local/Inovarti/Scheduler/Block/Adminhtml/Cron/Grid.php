 ...
 */
class Inovarti_Scheduler_Block_Adminhtml_Cron_Grid extends Mage_Adminhtml_Block_Widget_Grid {
	 ...
	 *
	 * @return Inovarti_Scheduler_Block_Adminhtml_Cron_Grid Self
	 */
	protected function _prepareCollection() {
		$collection = Mage::getModel('inovarti_scheduler/collection_crons');
	...
	protected function _prepareMassaction() {
		...
		$this->getMassactionBlock()->addItem('schedule', array(
			'label'         => Mage::helper('inovarti_scheduler')->__('Schedule now'),
			'url'           => $this->getUrl('*/*/scheduleNow'),
		));
		$this->getMassactionBlock()->addItem('run', array(
			'label'    => Mage::helper('inovarti_scheduler')->__('Run now'),
			'url'      => $this->getUrl('*/*/runNow'),
		));
		$this->getMassactionBlock()->addItem('disable', array(
			'label'    => Mage::helper('inovarti_scheduler')->__('Disable'),
			'url'      => $this->getUrl('*/*/disable'),
		));
		$this->getMassactionBlock()->addItem('enable', array(
			'label'    => Mage::helper('inovarti_scheduler')->__('Enable'),
	...
	protected function _prepareColumns() {
		...
		$this->addColumn('cron_expr', array (
			'header' => Mage::helper('inovarti_scheduler')->__('Cron Expression'),
			'index' => 'cron_expr',
			'sortable'  => false,
		));
		$this->addColumn('model', array (
			'header' => Mage::helper('inovarti_scheduler')->__('Model'),
			'index' => 'model',
			'sortable'  => false,
		));
	...
	public function decorateStatus($value) {
		$cell = sprintf('<span class="grid-severity-%s"><span>%s</span></span>',
			($value == Inovarti_Scheduler_Model_Configuration::STATUS_DISABLED) ? 'critical' : 'notice',
			Mage::helper('inovarti_scheduler')->__($value)
		);
		return $cell;
	}
