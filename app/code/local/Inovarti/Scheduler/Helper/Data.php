 ...
 */
class Inovarti_Scheduler_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	public function decorateTime($value, $echoToday=false, $dateFormat=NULL) {
		if (empty($value) || $value == '0000-00-00 00:00:00') {
			...
			$replace = array(
				Mage::getModel('core/date')->date('Y-m-d ', time()) => $echoToday ? Mage::helper('inovarti_scheduler')->__('Today') . ', ' : '', // today
				Mage::getModel('core/date')->date('Y-m-d ', strtotime('+1 day')) => Mage::helper('inovarti_scheduler')->__('Tomorrow') . ', ',
				Mage::getModel('core/date')->date('Y-m-d ', strtotime('-1 day')) => Mage::helper('inovarti_scheduler')->__('Yesterday') . ', ',
			);
	...
	public function getLastHeartbeat() {
		if ($this->isDisabled('inovartischeduler_heartbeat')) {
			return false;
		}
		...
		$schedules->addFieldToFilter('job_code', 'inovartischeduler_heartbeat');
		$schedules->load();
