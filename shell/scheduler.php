...
class Inovarti_Scheduler_Shell_Scheduler extends Mage_Shell_Abstract {
	...
	public function listAllCodesAction() {
		...
		foreach ($collection as $configuration) { /* @var $configuration Inovarti_Scheduler_Model_Configuration */
	...
	public function lastRunAction() {
		...
		$collection->addFieldToFilter('job_code', $code)
			...
			->getSelect()->limit(1);
		$schedule = $collection->getFirstItem(); /* @var $schedule Inovarti_Scheduler_Model_Schedule */
...
$shell = new Inovarti_Scheduler_Shell_Scheduler();
$shell->run();